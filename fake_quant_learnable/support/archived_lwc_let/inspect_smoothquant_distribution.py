...
from .modules import BaselineFakeQuantLinear, FrozenLearnedFakeQuantLinear
...
def activation_quant_input(wrapper: nn.Module, wrapper_input: torch.Tensor) -> torch.Tensor:
    if isinstance(wrapper, FrozenLearnedFakeQuantLinear):
        return wrapper._let_activation(wrapper_input)
