...
from .apply import BaselineQuantSummary
...
from .tail_protect import TailProtectedBaselineFakeQuantLinear, apply_tail_protected_w8a8
from benchmark.tasks.v1_0.registry import get_task_config


DEFAULT_TAIL_PROTECT_OUTPUT_DIR = "fake_quant_learnable/results/w8a8_tail_protect_ad"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run AD W8A8 baseline with tail-token activation protection."
    )
    parser.add_argument("--model_path", default=DEFAULT_MODEL_PATH)
    parser.add_argument("--data_dir", default=DEFAULT_DATA_DIR)
    parser.add_argument("--output_dir", default=DEFAULT_TAIL_PROTECT_OUTPUT_DIR)
    parser.add_argument("--layers", default="all")
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--eval_sample_size", default="full")
    parser.add_argument("--tail_tokens", type=int, default=1)
...
def apply_tail_protected_layers(
    *,
    model: nn.Module,
    ...
    for layer_idx in layer_indices:
        ...
        if isinstance(layer, nn.Linear):
            layers[layer_idx] = TailProtectedBaselineFakeQuantLinear(
                layer,
            ...
            summary = apply_tail_protected_w8a8(
                layer,
                act_quant=DEFAULT_ACT_QUANT,
        ...
        print(
            f"[w8a8_tail_protect] layer={layer_idx} replaced_linears={summary.replaced_linears} "
            f"tail_tokens={tail_tokens} shared_attention_modules={summary.shared_attention_modules} "
...
def main() -> None:
    ...
    if args.tail_tokens < 0:
        raise ValueError(f"--tail_tokens must be non-negative, got {args.tail_tokens}")
    set_seed(args.seed)
    ...
    baseline_summaries = apply_tail_protected_layers(
        model=model,
    ...
    config: dict[str, Any] = {
        ...
        "seed": args.seed,
        "histories": histories_to_jsonable({}),
        "baseline_summaries": summaries_to_jsonable(baseline_summaries),
    }
    (output_file.parent / "w8a8_tail_protect_config.json").write_text(
    ...
    for sample_id, sample in tqdm(
        ...
        desc="w8a8_tail_protect AD generation",
    ):
